...
function AddStockModal({ open, onClose, onSubmit, loading }) {
  ...
  const validate = () => {
    ...
    if (!form.currentStock || isNaN(form.currentStock)) errs.currentStock = 'Current stock is required and must be a number';
    if (!form.minimumThreshold || isNaN(form.minimumThreshold)) errs.minimumThreshold = 'Minimum threshold is required and must be a number';
    if (!form.stock) errs.stock = 'Stock status is required';
    if (!form.pricePerKg || isNaN(form.pricePerKg)) errs.pricePerKg = 'Price per Kg is required and must be a number';
    if (!form.file) errs.file = 'Product image is required (JPG or PNG)';
    else if (form.file && !['image/jpeg', 'image/png'].includes(form.file.type)) errs.file = 'Only JPG or PNG images are allowed';
    setErrors(errs);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/10">
      <div className="bg-white rounded shadow-lg p-6 w-full max-w-md relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-sm font-medium">Current Stock (Kg)</label>
            <input name="currentStock" value={form.currentStock} onChange={handleChange} className="w-full border rounded px-2 py-1" type="number" />
            {errors.currentStock && <div className="text-red-500 text-xs">{errors.currentStock}</div>}
          </div>
          <div>
            <label className="block text-sm font-medium">Minimum Threshold (Kg)</label>
            <input name="minimumThreshold" value={form.minimumThreshold} onChange={handleChange} className="w-full border rounded px-2 py-1" type="number" />
            {errors.minimumThreshold && <div className="text-red-500 text-xs">{errors.minimumThreshold}</div>}
          </div>
          <div>
            <label className="block text-sm font-medium">Stock Status</label>
            <select name="stock" value={form.stock} onChange={handleChange} className="w-full border rounded px-2 py-1">
          ...
          <div>
            ...
            <input name="pricePerKg" value={form.pricePerKg} onChange={handleChange} className="w-full border rounded px-2 py-1" type="number" />
            {errors.pricePerKg && <div className="text-red-500 text-xs">{errors.pricePerKg}</div>}
          </div>
          <div>
            <label className="block text-sm font-medium">Product Image <span className='text-red-500'>(Upload JPG or PNG)</span></label>
            <input name="file" type="file" accept="image/jpeg,image/png" onChange={handleChange} className="w-full" />
...
export default function StockDetails() {
  ...
  const handleAddStock = async (form, resetForm) => {
    ...
    try {
      ...
      resetForm({ name: '', category: '', currentStock: '', minimumThreshold: '', stock: '', pricePerKg: '', file: null });
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/10">
        <div className="bg-white rounded shadow-lg p-6 w-full max-w-md relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-3" encType="multipart/form-data">
            ...
            <div>
              <label className="block text-sm font-medium">Product Image <span className='text-gray-500'>(Upload JPG or PNG to update)</span></label>
              <input name="file" type="file" accept="image/jpeg,image/png" onChange={handleChange} className="w-full" />
            </div>
            <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded mt-2" disabled={loading}>
    ...
    <div className="bg-white shadow-sm rounded-sm p-4 m-3 text-gray-700">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button className="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded text-sm" onClick={() => setShowAddModal(true)}>
            + Add Stock
          </button>
          <button
            onClick={exportToCSV}
            ...
            {loading ? (
              ...
              <tr key={idx} className="hover:bg-gray-50 ">
                ...
                <td className="p-2 flex gap-3 items-center">
                  <span className="text-blue-600 cursor-pointer" onClick={() => setEditProduct(item)}>View / Edit</span>
                  <button
                    className="text-red-600 hover:text-red-800 disabled:opacity-50"
                    onClick={() => handleDeleteProduct(item._id, item.public_id)}
      ...
      <AddStockModal open={showAddModal} onClose={() => setShowAddModal(false)} onSubmit={handleAddStock} loading={addLoading} />
      <EditStockModal open={!!editProduct} onClose={() => setEditProduct(null)} product={editProduct} onSubmit={handleEditStock} loading={editLoading} />
    </div>
  );
}
